(function(){
  ...
  while (counter < files.length) {
    try {
      result = fs.readFileSync("famous.wiki/" + files[counter] + ".md", 'utf-8');
    } catch (e$) {
      e = e$;
  ...
  prev = "";
  for (i$ = 0, len$ = (ref$ = "Famous.docset/Contents/Resources/Documents/css".split("/")).length; i$ < len$; ++i$) {
    dir = ref$[i$];
  ...
  for (i$ = 0, len$ = files.length; i$ < len$; ++i$) {
    file = files[i$];
    if (fs.existsSync("Famous.wiki/" + file + ".md")) {
      md_data = "" + fs.readFileSync("Famous.wiki/" + file + ".md", 'utf-8');
      ...
      fs.writeFileSync("Famous.docset/Contents/Resources/Documents/" + file + ".html", final_data, 'utf-8');
    }
  }
  api = fs.readFileSync("Famous.wiki/API-Reference.md", 'utf-8');
  link_1st = /\[\[([\w\.-]+)\|([\w-]+)(#([\w-\.]+))?\]\]/g;
  link_2nd = /\[([\w\.-]+)\]\(([\w-]+)(#([\w-\.]+))?\)/g;
  full_results = [];
  ...
  for (i$ = 0, len$ = full_results.length; i$ < len$; ++i$) {
    ...
    if (result[1].indexOf("Famous.") === 0 && result[1].match(/\./g).length === 1) {
  ...
  fs.createReadStream('github.css').pipe(fs.createWriteStream('Famous.docset/Contents/Resources/Documents/css/github.css'));
  fs.createReadStream('Info.plist').pipe(fs.createWriteStream('Famous.docset/Contents/Info.plist'));
  fs.createReadStream('icon.png').pipe(fs.createWriteStream('Famous.docset/icon.png'));
  for (i$ = 0, len$ = (ref$ = fs.readdirSync("Famous.wiki/")).length; i$ < len$; ++i$) {
    file = ref$[i$];
    if (file.indexOf('.png') !== -1) {
      fs.createReadStream("Famous.wiki/" + file).pipe(fs.createWriteStream("Famous.docset/Contents/Resources/Documents/" + file));
    }
  }
